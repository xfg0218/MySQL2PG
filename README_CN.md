...
### 🔧 便捷的开发体验
- **assess 评估模式**：v3.4.0 新增！迁移前兼容性评估，生成详细的 HTML 报告，包含风险等级和建议
...
# 评估将执行以下操作：
# 1. 测试 MySQL 和 PostgreSQL 连接
# 2. 分析所有表、视图、函数、索引、用户和权限
# 3. 生成兼容性报告和风险等级
# 4. 创建 HTML 评估报告（assessment-YYYY-MM-DD_HHmmss.html）
```

评估报告包含：
- **总体评分**：0-100 的兼容性评分
- **风险等级**：基于不兼容对象数量的低/中/高风险等级
- **详细清单**：表、视图、函数、索引、用户、权限的风险评估
- **风险描述**：每个对象的具体不兼容问题和建议
