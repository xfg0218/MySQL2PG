...
- **多维度数据校验**：同步后自动验证数据一致性，校验准确率100%，支持批量校验和增量校验
- **数据不一致检测**：自动统计数据量不匹配的表，提供详细的不一致表清单
- **灵活同步策略**：支持全量同步和增量同步，可配置同步前是否清空表数据
...
```bash
# 克隆仓库
git clone https://github.com/yourusername/mysql2pg.git
cd mysql2pg
...
- **使用场景**：
  ...
  - `false`：适用于增量同步场景，保留PostgreSQL中已有的数据
...
- **使用场景**：在增量同步场景中，了解哪些表的数据量不一致，以便后续进行处理
...
conversion:
  options:
    validate_data: true         # 确保数据一致性
    truncate_before_sync: false # 增量同步，保留已有数据
...
```
同步表 user 数据成功，共有 327680 行数据，数据校验不一致
同步表 users_20251201 数据成功，共有 200002 行数据，数据校验不一致
