...
func (m *Manager) syncTableData(tables []mysql.TableInfo, semaphore chan struct{}) error {
	for _, table := range tables {
		...
		// 获取表列信息
		columns, err := m.mysqlConn.GetTableColumns(table.Name)
		if err != nil {
		...
		// 获取表数据总行数
		totalRows, err := m.mysqlConn.GetTableRowCount(table.Name)
		if err != nil {
		...
		// 开始事务
		tx, err := m.postgresConn.BeginTransaction(context.Background())
		if err != nil {
		...
		// 获取批量大小配置
		batchSize := int64(m.config.Conversion.Limits.MaxRowsPerBatch)
		if batchSize <= 0 {
			batchSize = 1000 // 默认值
		}

		batchInsertSize := m.config.Conversion.Limits.BatchInsertSize
		if batchInsertSize <= 0 {
			batchInsertSize = 1000 // 默认值
		}
		...
		primaryKey, err = m.mysqlConn.GetTablePrimaryKey(table.Name)
		if err != nil {
			m.Log("警告: %v，将使用传统的OFFSET分页", err)
			useKeyPagination = false
		} else {
			m.Log("表 %s 的主键是 %s，将使用基于主键的分页", table.Name, primaryKey)
		...
		for processedRows < totalRows {
			...
			if useKeyPagination {
				// 使用基于主键的分页
				rows, err = m.mysqlConn.GetTableDataWithPagination(table.Name, columns, primaryKey, lastValue, int(batchSize))
			} else {
				// 使用传统的OFFSET分页
				offset := processedRows
				rows, err = m.mysqlConn.GetTableData(table.Name, columns, int(offset), int(batchSize))
			}
			...
			// 使用批量插入并获取实际处理的行数
			currentBatchSize, lastValue, err = m.postgresConn.BatchInsertDataWithTransactionAndGetLastValue(tx, table.Name, columns, batchInsertSize, primaryKey, rows)
			rows.Close() // 确保关闭rows
			...
			// 生成动态进度条
			generateProgressBar := func(progress float64, length int) string {
				filled := int(float64(length) * progress / 100)
				if filled > length {
					filled = length
				}
				bar := "["
				for i := 0; i < length; i++ {
					if i < filled {
						bar += "-"
					} else if i == filled && filled < length {
						bar += "->"
						i++ // 跳过下一个位置，因为箭头占用了两个字符
					} else {
						bar += " "
					}
				}
			...
			if m.config.Run.ShowConsoleLogs {
				m.mutex.Lock()
				overallProgress := float64(m.completedTasks) / float64(m.totalTasks) * 100
				currentTask := m.completedTasks + 1
				m.mutex.Unlock()
				...
				fmt.Printf("\r进度: %.2f%% (%d/%d) : 同步表 %s %s %.2f%%", overallProgress, currentTask, m.totalTasks, table.Name, progressBar, progress)
			}
		}
		...
		if m.config.Conversion.Options.ValidateData {
			pgRowCount, err := m.postgresConn.GetTableRowCount(table.Name)
			if err != nil {
			...
			if pgRowCount == totalRows {
				if m.config.Conversion.Options.TruncateBeforeSync {
					// 当truncate_before_sync为true时，显示数据校验一致
					validationResult = "，数据校验一致"
					m.Log("校验表 %s 数据成功，MySQL和PostgreSQL行数一致", table.Name)
				} else {
					// 当truncate_before_sync为false时，如果行数相同，显示数据校验一致
					validationResult = "，数据校验一致"
					m.Log("校验表 %s 数据成功，MySQL和PostgreSQL行数一致", table.Name)
				}
			} else {
				...
				if m.config.Conversion.Options.TruncateBeforeSync {
					// 当truncate_before_sync为true时，数据不一致返回错误
					if m.config.Run.ShowConsoleLogs {
						fmt.Printf("\n进度: %.2f%% (%d/%d) : 同步表 %s 数据失败，共有 %d 行数据，请检查数据量\n", progress, m.completedTasks, m.totalTasks, table.Name, totalRows)
					}
					// m.logError(fmt.Sprintf("校验表 %s 数据失败，MySQL行数: %d, PostgreSQL行数: %d", table.Name, totalRows, pgRowCount))
					<-semaphore
					return fmt.Errorf("校验表 %s 数据失败，行数不一致", table.Name)
				} else {
					// 当truncate_before_sync为false时，数据不一致不中断，显示数据校验不一致
					validationResult = "，数据校验不一致"
				}
			}
		}
		...
		if m.config.Run.ShowConsoleLogs {
			fmt.Printf("\n进度: %.2f%% (%d/%d) : 同步表 %s 数据成功，共有 %d 行数据%s\n", progress, m.completedTasks, m.totalTasks, table.Name, totalRows, validationResult)
		}
...
func (m *Manager) convertTablePrivilegesNew(tablePrivileges []mysql.TablePrivInfo, semaphore chan struct{}) error {
	for _, tablePriv := range tablePrivileges {
		...
		// 显示转换成功信息（根据配置决定是否在控制台显示）
		if m.config.Run.ShowConsoleLogs {
			fmt.Printf("进度: %.2f%% (%d/%d) : 转换表权限成功\n", progress, m.completedTasks, m.totalTasks)
		}
