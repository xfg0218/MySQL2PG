...
var (
	...
	// 修改：使用 ([\s\S]*?) 匹配表达式，支持括号嵌套
	rePartitionCommentRange = regexp.MustCompile(`(?is)/\*![0-9]{5,}\s+PARTITION\s+BY\s+RANGE\s*\(([\s\S]*?)\)\s*\(([\s\S]*?)\)\s*\*/`)
	// RANGE + SUBPARTITION 子分区：/*!XXXXX PARTITION BY RANGE (expr) SUBPARTITION BY HASH (expr2) SUBPARTITIONS N (PARTITION ...) */
	rePartitionCommentRangeSub = regexp.MustCompile(`(?is)/\*![0-9]{5,}\s+PARTITION\s+BY\s+RANGE\s*\(([\s\S]*?)\)\s*SUBPARTITION\s+BY\s+(HASH|KEY)\s*\(([^)]+)\)\s*SUBPARTITIONS\s+(\d+)\s*\(([\s\S]*?)\)\s*\*/`)
	
	// LIST 分区：/*!XXXXX PARTITION BY LIST (expr) (PARTITION ...) */
	// 支持多值列表：VALUES IN (1,2,3) 或 VALUES IN ('a','b')
	rePartitionCommentList  = regexp.MustCompile(`(?is)/\*![0-9]{5,}\s+PARTITION\s+BY\s+LIST\s*\(([\s\S]*?)\)\s*\(([\s\S]*?)\)\s*\*/`)
