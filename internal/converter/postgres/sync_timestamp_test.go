...
// TestHasTimestampColumn 验证 TIMESTAMP 列检测（排除 DATETIME、CURRENT_TIMESTAMP 默认值、表级选项）
func TestHasTimestampColumn(t *testing.T) {
	tests := []struct {
		...
		{
			name: "CURRENT_TIMESTAMP 默认值不算",
			ddl:  "CREATE TABLE `t` (\n  `dt` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP\n)",
			want: false,
		},
		{
			name: "表级选项与注释不算",
			ddl:  "CREATE TABLE `t` (\n  `id` int\n) ENGINE=InnoDB COMMENT='timestamp is great';",
			want: false,
		},
		{
			name: "列名不带反引号",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := HasTimestampColumn(tt.ddl); got != tt.want {
				t.Errorf("HasTimestampColumn() = %v, want %v", got, tt.want)
			}
		})
	}
}
