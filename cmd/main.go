...
func showHelp() {
	...
	fmt.Println("  max_open_conns: 连接池配置的最大连接数 (默认: 50)")
	fmt.Println("  max_idle_conns: 连接池配置的最大空闲连接数 (默认: 20)")
	...
	fmt.Println("  test_only: 仅测试连接，不执行转换 (默认: false)")
	fmt.Println("  max_conns: 连接池配置的最大连接数 (默认: 20)")
	fmt.Println()
	fmt.Println("转换选项:")
	...
	fmt.Println("  skip_existing_tables: 如果表在PostgreSQL中已存在则跳过，否则创建 (默认: true)")
	...
	fmt.Println("  lowercase_columns: 控制表字段是否需要转小写 (默认: true)")
